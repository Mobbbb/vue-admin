<template>
	<div>
        ...
        <div v-show="current === 0" class="form-body">
            <Form ref="basicForm" :model="params" :rules="userRules" :label-width="100">
                <FormItem label="用户群名称 :" prop="userGroupName">
                    <Input :maxlength="15" v-model="params.userGroupName" placeholder="请输入用户群名称" style="width: 250px"></Input> 
                </FormItem>
                <FormItem label="用户群类型 :" prop="userGroupType">
                    <Select v-model="params.userGroupType" style="width: 250px">
                        <Option value="1">导入类</Option>
                        <Option value="2">条件类</Option>
                    </Select>
                </FormItem>
            </Form>
        </div>
                ...
                <Form :model="cityConfig" ref="cityCheck" :rules="cityConfigRules">
                    <table>
                        <tr>
                            ...
                            <td>
                                <div v-if="checkStatus.cityCheck">
                                    城市 ：
...
import { addUserGroup, pollingUploadResult, userGroupConfirm } from '@/api/marketing'
...
export default {
    ...
    data() {
        ...
        return {
            ...
            percent: 0,             // 进度条百分比
            userRules: {            // 校验步骤一
                userGroupName: [{ required: true, trigger: 'change', message: '用户群名称不得为空' }],
                userGroupType: [{ required: true, trigger: 'change', message: '请选择用户群类型' }],
            },
    ...
    methods: {
        ...
        getImportResult (response) { // 获取导入类结果
            ...
            addUserGroup(params).then(res => { // save接口
